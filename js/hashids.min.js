...
{ 
   ...
   {   
   	...
   	{
   		var i,len,numbers;str=str.toString();
   	...
   	Hashids.prototype._decode=function(hash,alphabet)
   	{
   		...
   		if(typeof hashBreakdown[0]!=="undefined")
   		{
   			...
   			{
   				subHash=hashArray[i];buffer=lottery+this.salt+alphabet;
   				alphabet=this.consistentShuffle(alphabet,buffer.substr(0,alphabet.length));
   	...
   	{
   		var integer,j,temp,i,v,p;if(!salt.length)
   		{
   			return alphabet
   		}
   		for(i=alphabet.length-1,v=0,p=0;i>0;i--,v++)
   		{
   			v%=salt.length;p+=integer=salt.charAt(v).charCodeAt(0);
   			j=(integer+v+p)%i;
   		...
   		Hashids.prototype.hash=function(input,alphabet)
   		{
   			...
   			do
   			{
   				hash=alphabet.charAt(input%alphabetLength)+hash;input=parseInt(input/alphabetLength,10)
   			}
   			while(input);
   			return hash
   		};
   		Hashids.prototype.unhash=function(input,alphabet)
   		{
   			...
   			{
   				define(function(){return Hashids})
   			}
   			return Hashids
   		}();